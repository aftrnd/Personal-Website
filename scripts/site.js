  ...
  window.Site = Singleton.create({
    ...
    setupNavigation: function() {
      ...
      function setPageHeight() {
        var sidebarHeight = 0;
        var headerH = 0;
        if (Y.one('#sidebarWrapper') && parseInt(Y.one('#sidebarWrapper').getComputedStyle('height'),10)) {
          sidebarHeight = parseInt(Y.one('#sidebarWrapper').getComputedStyle('height'),10);
        }
        if (Y.one('#headerWrapper') && parseInt(Y.one('#headerWrapper').getComputedStyle('height'),10)) {
    ...
    setupGalleries: function() {
      ...
      if (this.getBodyWidth() < MOBILE_GALLERY_BREAKPOINT) {
        ...
        var hash = Y.config.win.location.hash;
        if (hash) {
          var imageId = hash.replace(/\#itemId\=/, '');
          var imageEl = Y.one('[data-slide-id="' + imageId + '"]');

          // We have to get a little hacky to allow deep linking on mobile in
          // this template. The interval detects when the image is loaded.
          // The timeout gives a little padding to allow the layout to reflow.
          // kfoley
          var imagesLoadedInterval = setInterval(function() {
            if (imageEl.one('img').hasClass('loaded')) {
              setTimeout(function() {
                window.scrollTo(0, imageEl.getY());
              }, 200);
        ...
        if (Y.one('body.enable-gallery-thumbnails.initial-gallery-view-thumbnails') || Y.one('body.enable-gallery-thumbnails.homepage.homepage-gallery-view-thumbnails')) {
          if (!Y.one('body.enable-gallery-thumbnails.homepage.homepage-gallery-view-slideshow')) {
            var shouldDisplaySlideshow = window.location.hash;
            this.setupSlideshow();
            if (shouldDisplaySlideshow) {
    ...
    checkHeaderHeight: function() {
      ...
      if (Y.all('#header .subnav').size() > 0) {
        Y.all('#header .subnav').each(function(n) {
